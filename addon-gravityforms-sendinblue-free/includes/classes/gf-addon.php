...
class GF_Addon extends GFFeedAddOn
{
		...
		return array(
			array(
				...
				'fields' => array(
					...
					array(
						'name'     => 'sib-api-key-field',
						'type'     => 'custom_html',
						'callback' => [$this, 'settings_sib_api_key_field'],
					),
				),
			),
		);
	...
	/**
	 * Render the Brevo API Key field and error/success messages.
	 *
	 * @return void
	 */
	public function settings_sib_api_key_field()
	{
		$api_key = dkgfsib_free_get_addon()->get_plugin_setting('sib-api-key');
		$this->validate_sendinblue_api_key_field($api_key);

		$error = Options\get_plugin_option('sib_api_error');
		...
		} elseif (dkgfsib_free_api_key_is_valid()){
			printf(
				'<div class="notice inline notice-success"><p>%1$s</p></div>',
				__('API Key is valid.', 'addon-gravityforms-sendinblue-free')
			);
		}
	}
	...
	public function validate_sendinblue_api_key_field($value)
	{
		...
		if (is_wp_error($result)) {
			...
			if (!empty($error_data['pretty_response']) && isset($error_data['pretty_response']->message)) {
				$message_details = $error_data['pretty_response']->message;
			} else {
				$message_details = __('Unknown error occurred.', 'addon-gravityforms-sendinblue-free');
			}
	...
	public function feed_settings_fields()
	{
		if (! dkgfsib_free_api_key_is_valid()) {
			...
			if (is_array($lists) && ! empty($lists)) {
				...
				$fields[] = array(
					'name'    => 'enable-note-mapping',
					'label'   => esc_html__('Create a note (Pro version)', 'addon-gravityforms-sendinblue-free'),
					'type'    => 'checkbox',
					'tooltip' => esc_html__('By enabling this feature it will be possible to add notes to a created/updated user', 'addon-gravityforms-sendinblue-free'),
					'choices' => array(
						array(
							'label'   => esc_html__('Enable note creation. (Pro version)', 'addon-gravityforms-sendinblue-free'),
							'name'    => 'enable-note-mapping-enabled',
							'disabled' => true, 
						),
					),
					'dependency' => $is_enabled_dependency,
				...
				$fields[] = array(
					'name'    => 'partial-entries-enabled',
					'label'   => esc_html__('Use partial entries (Pro version)', 'addon-gravityforms-sendinblue-free'),
					'type'    => 'checkbox',
					'tooltip' => sprintf(
						esc_html__("Allows sending partial entries to Brevo. Make sure to enable partial entries for this form in the Gravity Forms settings.", 'addon-gravityforms-sendinblue-free'),
					),
					'choices' => array(
						array(
							'label'   => esc_html__('Enable partial entries. (Pro version)', 'addon-gravityforms-sendinblue-free'),
							'name'    => 'partial-entries-enabled',
							'disabled' => true,
						),
					),
